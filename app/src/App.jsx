...
class App extends React.Component {
  ...
  loadSubjectData(subject) {
    this.subjectData = new SubjectData(
      this.loadServerFile(SUBJECTS_FILEPATHS[subject])
    );
  }
  ...
  levenshtein(text0, text1) {
    let levenDist = Array(text0.length + 1)
      .fill()
    ...
    for (let i = 1; i <= text0.length; i++) {
      for (let j = 1; j <= text1.length; j++) {
        levenDist[i][j] = Math.min(
          levenDist[i - 1][j] + 1,
          levenDist[i][j - 1] + 1,
          levenDist[i - 1][j - 1] + (text0[i - 1] === text1[j - 1] ? 0 : 1)
        );
      }
    }
  ...
  checkAnswer(answer) {
    ...
    if (
      this.state.questionAnswer
        ...
        .split(";")
        .some(
          (a) => this.levenshtein(a.trim(), answer.trim()) < answer.length / 5
        )
    ) {
      if (this.state.gameMode === "Rapid") {
        const newTime = this.state.time + 10;
        const minutes = ((newTime - (newTime % 60)) / 60)
          .toString()
          .padStart(2, "0");
    ...
    return (
      <div className="App bg-black text-white h-screen w-screen relative">
        {this.state.currentPage === "Landing" ? (
          <div className="Landing fixed overflow-scroll bg-black h-screen w-screen">
            ...
            <Button1
              label="LET'S PLAY"
              onClick={() => {
                this.setState({ currentPage: "Subject-Select" });
              }}
              styles="fixed 2xl:top-1/3 top-1/2 left-1/2 2xl:-translate-y-1/3 -translate-y-1/2 -translate-x-1/2 transition hover:bg-white hover:text-black hover:border-white"
            />
          </div>
        ) : null}
          ...
          <div className="Subject-Select fixed bg-black h-screen w-screen">
            <div className="flex gap-3 p-10 h-screen overflow-x-scroll overflow-y-hidden items-center">
              ...
              <SubjectTile
                ...
                icon={<MdGavel />}
                styles="bg-blue lg:bg-transparent hover:lg:bg-blue lg:text-blue hover:lg:text-white"
              ...
              <SubjectTile
                ...
                onClick={() => this.subjectHandler("AP Psychology")}
              />
              <SubjectTile
                subject="AP Biology"
                icon={<MdBiotech />}
                styles="bg-green lg:bg-transparent hover:lg:bg-green lg:text-green hover:lg:text-white"
                onClick={() => this.subjectHandler("AP Biology")}
              />
              <SubjectTile
                subject="Custom Flashcards"
                icon={<MdArrowCircleUp />}
                styles="bg-red lg:bg-transparent hover:lg:bg-red lg:text-red hover:lg:text-white"
                onClick={() => {
            ...
            <div className="flex flex-col h-screen w-screen justify-between items-center my-12">
              ...
              <Link
                href={
                  ...
                  "/data/" +
                  SUBJECTS_FILEPATHS["AP Government"]
                }
                text="Click to view an example"
              />
              <div>
                <Button1
                  label="Continue"
                  onClick={() => this.loadLocalData()}
                  styles="mt-3 transition hover:bg-white hover:text-black hover:border-white"
                />
              </div>
            </div>
          </div>
          ...
          <div className="Game-Mode-Select fixed bg-black h-screen w-screen flex flex-col">
            <div>
              <BackButton
                label="Back"
                onClick={() => {
                  this.setState({ currentPage: "Unit-Select", units: [] });
                }}
              />
            </div>
            <div className="grow flex flex-col lg:flex-row gap-3 p-10 h-full overflow-x-scroll overflow-y-hidden items-center place-content-center">
              <GameModeSelectCard
                ...
                styles="bg-dark-magenta lg:bg-transparent hover:lg:bg-dark-magenta lg:text-dark-magenta hover:lg:text-white"
                onClick={() => {
                  this.setState({ gameMode: "Rapid", currentPage: "Game" });
                }}
              />
              <GameModeSelectCard
                subject="Timed"
                icon={<MdOutlineTimer />}
                styles="bg-dark-orange lg:bg-transparent hover:lg:bg-dark-orange lg:text-dark-orange hover:lg:text-white"
                onClick={() => {
                  this.setState({ gameMode: "Timed", currentPage: "Game" });
                }}
              />
              <GameModeSelectCard
                subject="Zen"
        ...
        {this.state.currentPage === "Game" ? (
          <div
            className="Game fixed bg-black h-screen w-screen flex flex-col"
            onKeyDown={(e) => {
            ...
            {this.state.gameState === 0 ? (
              <div className="fixed h-full w-full bg-black">
                <Button1
                  ...
                  styles="fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 transition hover:bg-white hover:text-black hover:border-white"
                />
              </div>
            ) : null}
                ...
                <div className="fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2">
                  ...
                  <div>
                    <Button1
                      ...
                      styles="w-full mt-2 lg:w-auto transition hover:bg-white hover:text-black hover:border-white"
                    />
                    <Button1
                      label="Back"
                      onClick={() => {
                      ...
                      styles="w-full mt-2 lg:w-auto lg:ml-2 transition hover:bg-white hover:text-black hover:border-white"
                    />
                    <Button1
                      label="Quit"
                      onClick={() => {
            ...
            <div className="bottom-0 flex-1 flex flex-col md:flex-row-reverse gap-3 px-10 pb-10 md:pt-10 justify-center">
              ...
              <div className="grow flex flex-col max-w-4xl">
                ...
                <AnswerBox
                  ...
                  onChange={(t) =>
                    this.setState({ answerText: t.target.value })
                  }
                />
              </div>
